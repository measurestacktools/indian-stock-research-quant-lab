...
## Survivorship-Bias Protection (Phase 3)
`companies/securities` extended with `security_id, listed_date (inclusive), delisted_date (exclusive), source, retrieved_at, data_version` in `app/database/db.py:6` + `app/data/provider.py:6`; `security_symbols` tracks ticker history; `universe_snapshots` stores deterministic snapshots `as_of + definition + sorted symbols → hash` (`app/data/universe.py:1`). API `get_universe(as_of)` returns `listed_date ≤ as_of and (delisted_date is NULL or > as_of)` behind reusable interface, never hard-coded in backtester. Mock universe: `PENNY listed 2022-06-15` not eligible 2021, `MIDCAP delisted 2024-08-20` not on/after 2024-08-20, `OLDCO delisted 2023-01-15`. Backtester `app/backtesting/engine.py:13` checks `_is_in_universe` at signal and entry, forces exit on delist. Symbol changes via stable `security_id` (`app/data/symbol_history.py:1`), documented limitation (YFinance/Mock lack history). UI Data Health → Survivorship (eligible count, snapshot hash, timeline) inspectable; System shows survivorship ✓. Provenance preserved.
